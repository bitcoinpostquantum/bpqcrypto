...
#ifndef BPQ_XMSS_PRIVATEKEY_H_
#define BPQ_XMSS_PRIVATEKEY_H_
 ...
 **/
class XMSS_PrivateKey final 
	: public XMSS_PublicKey
   ...
   public:
    ...
    XMSS_PrivateKey(XMSS_Parameters::xmss_algorithm_t xmss_algo_id,
	  ...
	  secure_vector<uint8_t> sign(RandomNumberGenerator & rng, uint8_t const * msg, size_t msg_size);
      ...
      size_t size_long() const
         {
         return size() + XMSS_HashTree::size(XMSS_PublicKey::m_xmss_params);
         }
      ...
      void set_hashtree(const secure_vector<uint8_t>& raw_hashtree)
        {
        m_cached_hashtree = XMSS_HashTree(XMSS_PublicKey::m_xmss_params,
           raw_hashtree.data(), raw_hashtree.size());
        }

    secure_vector<uint8_t> raw_private_key_long() const
    {
		auto raw_key = raw_private_key();
		auto && raw_tree = m_cached_hashtree.raw_hashtree();

		secure_vector<uint8_t> key;
		key.reserve( 1 + raw_key.size() + raw_tree.size() );

		key.insert(key.end(), raw_key.begin(), raw_key.end());
		key.insert(key.end(), raw_tree.begin(), raw_tree.end());
   ...
   private:
       ...
       **/
      std::shared_ptr<Botan::Atomic<size_t>> recover_global_leaf_index() const;
      ...
      XMSS_HashTree m_cached_hashtree;
   };
