...
#ifndef BPQ_XMSS_PUBLICKEY_H_
#define BPQ_XMSS_PUBLICKEY_H_
...
namespace bpqcrypto {
	...
	using Botan::AlgorithmIdentifier;

	class XMSS_Verification_Operation;

	static const uint8_t BPQ_XMSS_PREFIX = 0x10;
...
class XMSS_PublicKey
   ...
   public:
       ...
       **/
      XMSS_PublicKey(XMSS_Parameters::xmss_algorithm_t xmss_oid,
                     const Botan::secure_vector<uint8_t>& root,
                     const Botan::secure_vector<uint8_t>& public_seed)
         : m_xmss_params(xmss_oid), m_wots_params(m_xmss_params.ots_oid()),
...
//         }
	  ...
	  bool verify(secure_vector<uint8_t> const & msg, secure_vector<uint8_t> const & signature);
...
//         }
      ...
      virtual size_t size() const
         {
         return 1 + sizeof(uint32_t) + 2 * m_xmss_params.element_size();
         }
